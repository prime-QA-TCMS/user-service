...
export const register = async (email: string, password: string, roleId: string, tenantId: string) => {
    ...
    if (!allowTestRegister && !tenantId) throw new Error("Tenant context is required");
...
export const login = async (email: string, password: string) => {
    ...
    if (role?.isDeleted || tenant?.isDeleted || tenant?.isActive === false) throw new Error("Invalid credentials");
    const permissions = (role.permissions || []).map((p: IPermission) => p.code.toLowerCase().replace(/_/g, "."));
...
export const refresh = async (token: string) => {
    ...
    if (!record || record.expiresAt < new Date() || record.revokedAt) throw new Error("Invalid refresh token");
    ...
    const role = user.role as any;
    const tenant = user.tenant as any;
    if (role?.isDeleted || tenant?.isDeleted || tenant?.isActive === false) throw new Error("User not found");
    const permissions = (role.permissions || []).map((p: IPermission) => p.code.toLowerCase().replace(/_/g, "."));
    const tenantId = tenant?._id ? tenant._id.toString() : tenant?.toString?.() || null;
