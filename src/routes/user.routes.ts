...
import { authenticate, requirePermission, requireTenant, success, AppError, ErrorCode, getUserContext } from "prime-qa-api-common";

const router = Router();

const allowSelfOrPermission = (permission: string) => async (req: any, res: any, next: any) => {
    try {
...
router.get("/", requirePermission("user.read"), async (req, res, next) => {
    try {
        ...
        const tenantId = (req as any).tenantId; // Set by requireTenant middleware
...
router.get("/:id", allowSelfOrPermission("user.read"), async (req, res, next) => {
    try {
        ...
        const userId = (req as any).user?.userId; // From authenticate middleware
        if (!tenantId) throw new AppError(ErrorCode.FORBIDDEN, "Tenant context is required", 403);
