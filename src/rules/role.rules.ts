...
export const ensureRoleNameUnique = async (name: string, tenantId: string) => {
    const existing = await RoleModel.findOne({ name, tenant: tenantId, isDeleted: false });
    if (existing) throw new Error("Role name must be unique per tenant");
};

export const ensureProtectedRoleName = async (roleId: string) => {
