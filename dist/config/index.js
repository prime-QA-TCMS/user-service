...
export const connectDb = async () => {
    try {
        ...
        const clientOpts = client?.options || {};
        let serverVersion;
        let replicaSet;
        try {
        ...
        if (readPreference !== undefined)
            console.log("[Mongo] ReadPref:", readPreference);
        if (retryWrites !== undefined)
            console.log("[Mongo] RetryWrites:", retryWrites);
        if (maxPoolSize !== undefined)
            console.log("[Mongo] MaxPoolSize:", maxPoolSize);
        if (directConnection !== undefined)
            console.log("[Mongo] DirectConnection:", directConnection);
    }
    catch (err) {
